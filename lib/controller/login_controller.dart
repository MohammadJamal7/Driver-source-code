    ...
    try {
      ...
      try {
        if (await googleSignIn.isSignedIn()) {
          ...
          // Also attempt disconnect in case a cached account exists
          await googleSignIn.disconnect();
          await googleSignIn.signOut();
        }
      } catch (_) {}

      // Ensure Firebase isn't holding onto a previous Google user
      try {
        await FirebaseAuth.instance.signOut();
      } catch (_) {}

      // Trigger interactive sign-in so the user can select the account
      ...
      if ((googleAuth.accessToken == null || googleAuth.accessToken!.isEmpty) &&
        ...
        // Defensive guard; tokens should not be both null/empty
        return null;
      }
      ...
      // Make sure app is no longer in guest mode on successful login
