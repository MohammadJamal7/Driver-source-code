...
class FreightController extends GetxController {
  ...
  Future acceptOrder(InterCityOrderModel orderModel) async {
    if (double.parse(driverModel.value.walletAmount.toString()) >=
      ...
      //
      ShowToastDialog.showToast("You have to minimum".tr +
          "${Constant.amountShow(amount: Constant.minimumDepositToRideAccept)}" +
          "wallet amount to Accept Order and place a bid".tr);
    }
  }
      ...
      location.requestPermission().then((permissionStatus) {
        if (permissionStatus == PermissionStatus.granted) {
          ...
          location.onLocationChanged.listen((locationData) async {
            ...
            if (currentUser != null) {
              FireStoreUtils.getDriverProfile(currentUser.uid).then((value) {
                DriverUserModel driverUserModel = value!;
                if (driverUserModel.isOnline == true) {
                  driverUserModel.location = LocationLatLng(
                      latitude: locationData.latitude,
              ...
              print(
                  "🚫 المستخدم غير مسجل دخول في FreightController location update");
            }
          });
        }
      });
